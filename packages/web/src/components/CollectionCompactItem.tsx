...
import { getCollectionInfo, type CollectionOutput } from '@yiwen-ai/store'
import { useCallback, useMemo } from 'react'
import { useIntl } from 'react-intl'
import { renderIconMoreAnchor } from './IconMoreAnchor'

export default function CollectionCompactItem({
  item,
  hasWritePermission,
  isRestoring,
  isDeleting,
    ...
    info && (
      ...
      >
        <div
          ...
          `}
        >
          <div css={textEllipsis}>{info.title}</div>
          {hasWritePermission && (
        ...
        >
          {intl.formatMessage(
            ...
            { date: new Date(item.updated_at ?? 0).toLocaleDateString() }
          )}
        </div>
      </div>
    )
